...
CoralCOTSMort = function(p,CoralCover) {
  (1 - (p*CoralCover/(1+CoralCover)))
}
CoralCOTSMort(3,seq(0,1, 0.01))

doPredPreyDynamics = function(season, year, COTSabund,KC, CoralCover, Crash, p) {
  ...
  # }
  # 
  COTSabund[,"A"] = 1 - (p*CoralCover/(1+CoralCover))
  return(COTSabund)
...
runModel = function(masterDF, PopData, COTS.data, Years = Years, data.grid, rep, Pred) {
  ...
  for(year in Years){  
    ...
    for(season in seasons){             # loop through seasons
      ...
      COTSabund = doPredPreyDynamics(season, year, COTSabund, KC, CoralCover, Crash, p=5)
